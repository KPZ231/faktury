...
// Get Robert's agent_id
$robertQuery = "SELECT agent_id FROM agenci WHERE imie = 'Robert' AND nazwisko = 'Kucharczyk'";
$robertStmt = $pdo->query($robertQuery);
$robertData = $robertStmt->fetch(PDO::FETCH_ASSOC);
$robertId = $robertData ? $robertData['agent_id'] : null;

// Get Kuba's agent_id
$kubaQuery = "SELECT agent_id FROM agenci WHERE imie = 'Kuba' AND nazwisko = 'Kucharczyk'";
$kubaStmt = $pdo->query($kubaQuery);
$kubaData = $kubaStmt->fetch(PDO::FETCH_ASSOC);
$kubaId = $kubaData ? $kubaData['agent_id'] : null;

// Define raty structure
$ratyOpis = ['Rata 1', 'Rata 2', 'Rata 3', 'Rata 4', 'Rata 5', 'Rata 6'];
...
while ($sprawa = $sprawyStmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $sprawa['oplaty']['Rata 4'] = [
        'kwota' => $sprawa['id_sprawy'] ? $pdo->query("SELECT installment4_amount FROM test2 WHERE id = {$sprawa['id_sprawy']}")->fetchColumn() : null,
        'is_paid_display' => $sprawa['id_sprawy'] ? (bool)$pdo->query("SELECT installment4_paid FROM test2 WHERE id = {$sprawa['id_sprawy']}")->fetchColumn() : false,
        'faktura_id' => null,
        'paying_invoice_number' => $sprawa['id_sprawy'] ? $pdo->query("SELECT installment4_paid_invoice FROM test2 WHERE id = {$sprawa['id_sprawy']}")->fetchColumn() : null
    ];
    
    // Rata 5
    $sprawa['oplaty']['Rata 5'] = [
        'kwota' => $sprawa['id_sprawy'] ? $pdo->query("SELECT installment5_amount FROM test2 WHERE id = {$sprawa['id_sprawy']}")->fetchColumn() : null,
        'is_paid_display' => $sprawa['id_sprawy'] ? (bool)$pdo->query("SELECT installment5_paid FROM test2 WHERE id = {$sprawa['id_sprawy']}")->fetchColumn() : false,
        'faktura_id' => null,
        'paying_invoice_number' => $sprawa['id_sprawy'] ? $pdo->query("SELECT installment5_paid_invoice FROM test2 WHERE id = {$sprawa['id_sprawy']}")->fetchColumn() : null
    ];
    
    // Rata 6
    $sprawa['oplaty']['Rata 6'] = [
        'kwota' => $sprawa['id_sprawy'] ? $pdo->query("SELECT installment6_amount FROM test2 WHERE id = {$sprawa['id_sprawy']}")->fetchColumn() : null,
        'is_paid_display' => $sprawa['id_sprawy'] ? (bool)$pdo->query("SELECT installment6_paid FROM test2 WHERE id = {$sprawa['id_sprawy']}")->fetchColumn() : false,
        'faktura_id' => null,
        'paying_invoice_number' => $sprawa['id_sprawy'] ? $pdo->query("SELECT installment6_paid_invoice FROM test2 WHERE id = {$sprawa['id_sprawy']}")->fetchColumn() : null
    ];
...
function syncPaymentStatuses($pdo) {
    ...
    $updateQuery = "UPDATE test2 t
                   ...
                   SET t.installment1_commission_paid = CASE WHEN cp.installment_number = 1 THEN cp.status ELSE t.installment1_commission_paid END,
                       ...
                       t.installment4_commission_paid = CASE WHEN cp.installment_number = 4 THEN cp.status ELSE t.installment4_commission_paid END,
                       t.installment5_commission_paid = CASE WHEN cp.installment_number = 5 THEN cp.status ELSE t.installment5_commission_paid END,
                       t.installment6_commission_paid = CASE WHEN cp.installment_number = 6 THEN cp.status ELSE t.installment6_commission_paid END
                   WHERE cp.status = 1";
...
<head>
    ...
    <style>
        ...
        .agent-payout:not(.zaplecono-faktura).do-wyplaty {
...
/* Wygląd dla Kuby - usunięte pomarańczowe podświetlenie */
    ...
    <table>
         <thead>
            <tr>
                ...
                <th>Ostatnia rata</th>
                <th colspan="<?php echo $liczbaRat; ?>">Prowizje agentów</th>
                <th>Ostatnia rata</th>
            </tr>
            <tr>
                <th>% Agenci</th>
                <th>% Do wypłaty Kuba</th>
                <?php foreach ($ratyOpis as $opisRaty): ?>
                    <th><?php echo $opisRaty; ?></th>
                <?php endforeach; ?>
                <th>Ostatnia rata</th>
                <?php foreach ($ratyOpis as $opisRaty): ?>
                    <th>Kwota do wyplaty agentowi</th>
                <?php endforeach; ?>
                <th>Kwota do wyplaty agentowi</th>
            </tr>
        </thead>
                ...
                <?php foreach ($sprawyData as $id_sprawy => $sprawa): ?>
                    <tr data-sprawa-id="<?php echo $id_sprawy; ?>">
                        ...
                        <td class="status-cell">
                            <?php if ($sprawa['czy_zakonczona']): ?>
                                <span class="status-zakonczona" title="Zakończona"><i class="fa-solid fa-circle-check"></i></span>
                            <?php else: ?>
                                <span class="status-niezakonczona" title="Niezakończona"><i class="fa-regular fa-circle"></i></span>
                            <?php endif; ?>
                        </td>
                        ...
                        <?php
                            ...
                            foreach ($ratyOpis as $opisRaty) {
                                $kwota = $sprawa['oplaty'][$opisRaty]['kwota'] ?? 0;
                                $suma_rat_z_bazy += $kwota;
                            }
                            $obliczona_ostatnia_kwota = max(0, $sprawa['calosc_prowizji'] - $suma_rat_z_bazy);
                            $statusClass = ($obliczona_ostatnia_kwota <= $epsilon) ? 'status-oplacona' : 'status-nieoplacona';
                            $statusSymbol = ($obliczona_ostatnia_kwota <= $epsilon) ? '☑' : '☐';
                        ?>
                        <td class="currency <?php echo $statusClass; ?>">
                            <?php echo format_currency($obliczona_ostatnia_kwota); ?>
                            <span class="checkbox"><?php echo $statusSymbol; ?></span>
                        </td>
                        ...
                        <?php foreach ($ratyOpis as $opisRaty): ?>
                            <td class="prowizje-rata-cell">
                                ...
                                <?php 
                                    ...
                                    if ($kwotaRaty > 0 && !empty($sprawa['prowizje_proc'])): 
                                        ...
                                        foreach ($sprawa['prowizje_proc'] as $agent_id => $proc): 
                                            if ($agent_id != $kubaId): // Pomiń Kubę w pierwszej pętli
                                                ...
                                                if ($kwotaProwizji > $epsilon):
                                                    ...
                                                    $payoutClass = '';
                                                    if ($czyOplacone) {
                                                        $payoutClass = 'zaplecono-faktura';
                                                    } elseif ($czyRataOplacona) {
                                                        $payoutClass = 'do-wyplaty';
                                                    }
                                ?>
                                                    <div class="agent-payout <?php echo $payoutClass; ?>" data-payment-id="payment_<?php echo $sprawa['id_sprawy']; ?>_<?php echo $opisRaty; ?>_<?php echo $agent_id; ?>">
                                                        ...
                                                        <?php if ($czyOplacone): ?>
                                                            <div class="status-zapłacono">
                                                                <?php if ($numerFaktury): ?>
                                                                    Zapłacono (Faktura: <?php echo htmlspecialchars($numerFaktury); ?>)
                                                                <?php else: ?>
                                                                    Zapłacono (Brak faktury)
                                                                <?php endif; ?>
                                                                <a href="#" class="edit-link" onclick="openPaymentModal('payment_<?php echo $sprawa['id_sprawy']; ?>_<?php echo $opisRaty; ?>_<?php echo $agent_id; ?>')">
                                ...
                                <?php 
                                        ...
                                        // Teraz wyświetl prowizję dla Kuby
                                        ...
                                        if ($kubaKwotaProwizji > $epsilon):
                                            ...
                                            $stmtCheck = $pdo->prepare("SELECT status as czy_oplacone, invoice_number as numer_faktury FROM commission_payments 
                                                WHERE case_id = ? AND agent_id = ? AND installment_number = ?");
                                            $installmentNumber = array_search($opisRaty, $ratyOpis) + 1; // Konwertuj nazwę raty na numer
                                            $stmtCheck->execute([$sprawa['id_sprawy'], $kubaId, $installmentNumber]);
                                            $platnosc = $stmtCheck->fetch(PDO::FETCH_ASSOC);
                                                ...
                                                <?php if ($czyOplacone): ?>
                                                    <div class="status-zapłacono">
                                                        ...
                                                        <?php else: ?>
                                                            Zapłacono (Brak faktury)
                                                        <?php endif; ?>
                                                        <a href="#" class="edit-link" onclick="openPaymentModal('payment_<?php echo $sprawa['id_sprawy']; ?>_<?php echo $opisRaty; ?>_kuba')">
                                                            <i class="fa-solid fa-pen-to-square"></i>
                                                        </a>
                                                    </div>
                                                <?php elseif ($czyRataOplacona && !$czyOplacone): ?>
                        ...
                        <!-- Ostatnia rata dla prowizji -->
                        <td class="prowizje-rata-cell">
                            <div class="details-section">
                            <?php 
                                if (!empty($sprawa['prowizje_proc'])): 
                                    // Najpierw wyświetl prowizje dla innych agentów
                                    foreach ($sprawa['prowizje_proc'] as $agent_id => $proc): 
                                        if ($agent_id != $kubaId): // Pomiń Kubę w pierwszej pętli
                                            // Oblicz kwotę prowizji dla ostatniej raty
                                            $kwotaProwizji = $obliczona_ostatnia_kwota * ($proc / 100);
                                            if ($kwotaProwizji > $epsilon):
                                                $uniqueId = "payment_{$sprawa['id_sprawy']}_ostatnia_{$agent_id}";
                            ?>
                                                <div class="agent-payout" data-payment-id="payment_<?php echo $sprawa['id_sprawy']; ?>_ostatnia_<?php echo $agent_id; ?>">
                                                    <span class="agent-name"><?php echo htmlspecialchars($agenci[$agent_id] ?? "Agent {$agent_id}"); ?>:</span>
                                                    <span class="agent-amount"><?php echo format_currency($kwotaProwizji); ?></span>
                                                </div>
                            <?php 
                                    ...
                                    // Teraz wyświetl prowizję dla Kuby
                                    $kubaKwotaProwizji = $obliczona_ostatnia_kwota * ($sprawa['do_wyplaty_kuba_proc'] / 100);
                                    if ($kubaKwotaProwizji > $epsilon):
                                        $kubaUniqueId = "payment_{$sprawa['id_sprawy']}_ostatnia_kuba";
                            ?>
                                        <div class="agent-payout" data-payment-id="payment_<?php echo $sprawa['id_sprawy']; ?>_ostatnia_kuba">
                                            <span class="agent-name">Kuba:</span>
                                            <span class="agent-amount"><?php echo format_currency($kubaKwotaProwizji); ?></span>
                                        </div>
                            <?php 
...
<!-- Dodaj ten element przed zamknięciem body -->
<div id="statusMessage" class="saving-status"></div>
...
function openPaymentModal(uniqueId) {
    ...
    const statusElement = agentPayoutElement.querySelector('.status-zapłacono');
    ...
    // Sprawdź czy wpis jest już oznaczony jako zapłacony
    const isPaid = statusElement !== null;
    ...
    const ratyOpisy = ['Rata 1', 'Rata 2', 'Rata 3', 'Rata 4', 'Rata 5', 'Rata 6'];
    const installmentNumber = ratyOpisy.indexOf(opis_raty) + 1; // Dodaj 1, aby numeracja była od 1
    
    // Wyczyść poprzednie statusy i ustaw domyślne wartości
    document.getElementById('modalPaidCheckbox').checked = isPaid;
...
function savePaymentInfoModal() {
    ...
    let kwota = 0;
    try {
        const amountText = currentPaymentData.amount.replace(/\s/g, '').replace(',', '.').replace('zł', '').trim();
        kwota = parseFloat(amountText);
        if (isNaN(kwota)) {
            console.error('Invalid amount format:', currentPaymentData.amount);
            kwota = 0;
        }
    } catch (e) {
    ...
    const paymentData = {
        ...
        agent_id: currentPaymentData.id_agenta.toLowerCase() === 'kuba' ? '1' : currentPaymentData.id_agenta,
